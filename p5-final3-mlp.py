...
#Class to track heat across frames   NOT USED, switched to collections.deque()
class TrackHeat:
    def __init__(self):
        self.hmap_list = []
...
#This is an experimental function developed to finetune detection accuracy without adding on to the feature vector load
#Particularly, the blue channel (2) makes a ready difference in detection.
def PreprocessImage (image, normalize):
    ...
    x, y, z = image.shape
    if ch0: red_total_avg = (np.sum(image [:,0,0])) / (x*y)    #Ch 0
    if ch1: green_total_avg = (np.sum(image [0,:,0]))  / (x*y) #Ch 1
    if ch2: blue_total_avg = (np.sum(image [0,0,:])) / (x*y)   #Ch 2
        
    #print ("Total RGB in the img: ", red_total_avg, green_total_avg, blue_total_avg)
    #Taking only the blue channel 
        
    #Multiply image by avgs
    if ch0: redmul = image*red_total_avg
    if ch1: greenmul = image*green_total_avg
    if ch2: bluemul = image*blue_total_avg
    #composite = redmul*greenmul*bluemul
        
    #plt.imshow(bluemul)
    #plt.show()
    if ch0: return redmul
    elif ch1: return greenmul
    elif ch2: return bluemul
...
def find_cars(img, ystart, ystop, scale, clf, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
    ...
    for xb in range(nxsteps):
        ...
        for yb in range(nysteps):
            ...
            #conf = clf.decision_function (test_features) #Distance from the separating SVM hyperplane, not a percentage
...
numframes = 12 #number of video frames to average over
heat_threshold = 2.5
scale = 1    
...
def ProcessImage (img):
    ...
    for s in scales:    
        boxes, hmap = find_cars(img, ystart, ystop, scale, clf, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
    ...
    history.append(hmap)
    array_trackHeat = np.array(history)
...
print ('1  Test Video    (test dataset, 1230/1530 car/noncar imgs/jpg)')
print ('2  Project Video (test dataset, 1230/1530 car/noncar imgs/jpg)')
print ('3  Test Video    (full dataset, 8800/9000 car/noncar imgs/png)')
print ('4  Project Video (full dataset, 8800/9000 car/noncar imgs/png)')
print ('5  Test Images')
...
t1 = time.time()
print ('Time to extract feature vector for training images: ', round((t1 - t0)/60., 2), 'min.')
...
# Check the training time for the DT
t=time.time()
clf.fit(X_train, y_train)
t2 = time.time()

print ('Using ', clfname)
print('Time reqd. to train clf: ', round(t2-t, 2), 'secs.')
...
scales = [1.0, 3.5]#, 2.0, 2.5]
#Instantiate object to track heat across frames
#track_heat = TrackHeat()   USING deque() instead
numframes = 12 #number of video frames to average over
heat_threshold = 2.5
font = cv2.FONT_HERSHEY_SIMPLEX
...
if args.video == '1':
    while True:
        ...
        framenum += 1        
    #choice2 = input("Like it?, Y/N")

if debug:
    textfile = open("runlog.txt","w")
    str1 = 'CLR spc ' + str(color_space) + 'orient ' + str(orient) + 'cell/blk ' + str(cell_per_block) + 'pix/cel '+ str(pix_per_cell) + ' ' + clfname + ' Choice: ' + str(choice)
    textfile.write(str1)
...
if args.video != '1':
    ...
    print ('Time reqd. to process and write to video file: ', round(time.time()-t2, 2)/60, 'min.')
