...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
    ...
    if scale != 1:
        ...
        print ('ctrans_tosearch, scale, ystart, ystop', ctrans_tosearch.shape, scale, ystart, ystop)
        ctrans_tosearch = cv2.resize(ctrans_tosearch, (np.int(imshape[1]/scale), np.int(imshape[0]/scale)))
        print ('ctrans_tosearch, ', ctrans_tosearch.shape, '\n')
    ...
    cells_per_step = 3  # Instead of overlap, define how many cells to step, same as overlap=0.75, 8 cells/blk, moving window 2 cells over = having 6 (out of 8) prev cells in the next blk
...
def draw_boxes1(img, bboxes, color=(0, 0, 255), thick=6):
    # Make a copy of the image
    imcopy = np.copy(img)
    random_color = False
    # Iterate through the bounding boxes
...
spatial_feat = True # Spatial features (resampling an img to become smaller or larger using OpenCV resize():on or off
...
#READ IMAGES DATASETS FOR TRAINING
...
for image in test_images:
    ...
    print (time.time() - t1, 'sec to process one img', len(windows), 'windows')
...
while True:
    ...
    if video:
        ...
        #for s in np.linspace(1.0, 3.0, 12): #iterating on scale in 0.5 increments, 1.0, 1.5, 2.0, ...
        
    ystart = 460
    ystop =  660
    scale =  3
    main_img, hmap = find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
                                spatial_feat, hist_feat, hog_feat)
    out_images.append(main_img)
    hmap_list.append(hmap)
    
    #imcopy = draw_boxes(main_img, windows)
    #plt.imshow(imcopy)
    #plt.show()
    
    
    ystart = 400 #(intention is to divide this space 100 px tall times 1280 across, by scale so: 4 windows each 640 wide by 50 tall)
    ystop  = 600
    scale  = 3
    main_img, hmap = find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
                                spatial_feat, hist_feat, hog_feat)
    ...
    for map in hmap_list:
        map = apply_threshold(hmap, 2)
        labels = label (hmap)
    ...
    out_images.append(main_img)
    out_images.append(hmap)
    framenum += 1
    #cv2.imshow('Vehicle Detection', draw_img)
    if cv2.waitKey(5) & 0xFF == ord('q'):
            break
print ("Total Detections (incl FPs): ", num_det)        
        
#fig = plt.figure (figsize=(12,24))
#visualize(fig, 35, 2, out_images, out_titles)
font = cv2.FONT_HERSHEY_SIMPLEX      
