...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
    ...
    if scale != 1.0:
        ...
        #print ('scale ne 1 blk: ctrans_tosearch, scale, ystart, ystop', ctrans_tosearch.shape, scale, ystart, ystop)
        ctrans_tosearch = cv2.resize(ctrans_tosearch, (np.int(imshape[1]/scale), np.int(imshape[0]/scale)))
        #print ('scale ne 1 blk: ctrans_tosearch, ', ctrans_tosearch.shape, '\n')
    #else: print ("Scale = 1 here")
    
    
    #Here, do the AveRGB
    #NEW STEP 
    ctrans_tosearch = PreprocessImage(ctrans_tosearch)
...
while True:
    ...
    draw_img = draw_labeled_boxes(np.copy(img1), labels)
    out_images.append(main_img)
    ...
    if cv2.waitKey(5) & 0xFF == ord('q'):
            break
print ("Total Detections (incl FPs): ", num_det, 'at scale ', scale)        
